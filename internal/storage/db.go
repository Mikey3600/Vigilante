...
func (db *DB) GetRecentLogsForTenant(ctx context.Context, tenantID string, limit int) ([]LogEntry, error) {
	rows, err := db.Pool.Query(ctx,
		`SELECT time, service_id, level, message, metadata FROM log_entries WHERE tenant_id = $1 ORDER BY time DESC LIMIT $2`,
		tenantID, limit)
	if err != nil {
		return nil, err
	}
...
func (db *DB) GetRecentMetricsForTenant(ctx context.Context, tenantID string, limit int) ([]MetricPoint, error) {
	rows, err := db.Pool.Query(ctx,
		`SELECT time, service_id, metric_name, value, labels FROM metric_points WHERE tenant_id = $1 ORDER BY time DESC LIMIT $2`,
		tenantID, limit)
	if err != nil {
		return nil, err
	}
